            ...
            <div className="flex gap-x-2 xl:flex-1 py-5">
                <div className="flexCenter flex-col gap-[7px] flex-wrap">
                    {product.images?.map((img, index) => (
                        <img key={index} src={url + "/images/" + img} alt={`productImg-${index}`} className='max-h-[89px] rounded-lg bg-gray-10' />
                    ))}
                </div>
            ...
            <div className="flex-col flex xl:flex-[1.5] bg-white px-6 py-2 rounded-xl">
                    ...
                    <div>
                        ...
                        <div className='flex gap-3 my-3'>
                            {product.colors?.map((color, index) => (
                                <div
                                    key={index}
                                    className={`h-10 w-10 flexCenter cursor-pointer rounded-sm ${color}`}
                                ></div>
                            ))}
                        </div>
                    </div>
                    <div>
                        ...
                        <div className="flex gap-3 my-3">
                            {product.sizes?.map((size, index) => (
                                <div key={index} className="border-[1.5px] border-slate-900/15 h-10 w-10 flexCenter cursor-pointer rounded-sm">{size}</div>
                            ))}
                        </div>
                    </div>
                </div>
                <div className="flex gap-4 mb-8 max-w-[555px] flex-wrap">
                    ...
                    <button onClick={() => navigate("/cart")}
                        className='btn-dark rounded-sm sm:!px-20 !py-2 flexCenter gap-x-4'
                    >
                    ...
                    {!cartItems[product._id] ? (
                        ...
                        <div className="bg-tertiary text-white rounded-sm flexCenter gap-2">
                            ...
                            <p className='text-white pr-2 '>{cartItems[product._id]}</p>
                            <FaPlus onClick={() => addToCart(product._id)} className="rounded-sm bg-secondary h-8 w-8 p-1 mr-1 cursor-pointer" />
                        </div>
                    )}
                </div>
                <p>
                ...
                <p>
                    <span className="medium-16 text-tertiary">Tags :</span> {product.tags?.join(' | ')}
                </p>
            </div>
        </section>
    )
