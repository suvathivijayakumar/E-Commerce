                ...
                <div className='absolute right-3 bottom-3 flexCenter gap-x-2'>
                    ...
                    {!cartItems[product._id] ? (
                        ...
                        <div className='bg-white rounded-full flexCenter gap-2 h-8'>
                            <FaMinus
                                onClick={() => removeFromCart(product._id)} className='rounded-full h-6 w-6 p-1 ml-1 bg-primary cursor-pointer' />
                            <p>{cartItems[product._id]}</p><FaPlus
                                onClick={() => addToCart(product._id)} className='rounded-full bg-secondary h-6 w-6 p-1 mr-1 cursor-pointer' />
                        </div>
                    )
                    }
                </div>
